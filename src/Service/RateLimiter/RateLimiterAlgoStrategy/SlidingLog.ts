...
export default class SlidingLog implements IRateLimitStrategy {
  constructor(private readonly policy: IPolicy) {}

  async check(user: IUserInfo): Promise<boolean> {
    const storeClient = new RedisStore().getRedisClient();
    const now = Date.now();
