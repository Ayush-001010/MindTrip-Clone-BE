...
class HotelService {
  ...
  async getHotels(
    ...
    const cacheKey = `explore:hotels:${normalizedCity}:${checkInDate}:${checkOutDate}`;
    const redisClient = this.redisStore.getRedisClient();
    let hotels: IHotel[];
    const cachedHotels = await redisClient.get(cacheKey);

    if (cachedHotels) {
      hotels = JSON.parse(cachedHotels) as IHotel[];
    } else {
      ...
      await redisClient.set(cacheKey, JSON.stringify(hotels), "EX", 3600);
    }
    let filteredHotels = hotels;

    if (minRating !== undefined) {
      filteredHotels = hotels.filter(
        (hotel) => hotel.rating !== undefined && hotel.rating >= minRating
      );
    }
